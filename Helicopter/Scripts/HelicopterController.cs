...
public class HelicopterController : MonoBehaviour
{
    ...
    private void OnKeyPressed(PressedKeyCode[] obj)
    {
		...
		//Joystick moving
		EngineForce += dir.z/2; // SpeedUp
		if (EngineForce < 0) EngineForce = 0;

		if (!IsOnGround) { 
			if (dir2.x > 0 && Mathf.Abs(dir2.x) > Mathf.Abs(dir2.z)) // right
				tempX = Time.fixedDeltaTime;
			if (dir2.x < 0 && Mathf.Abs(dir2.x) > Mathf.Abs(dir2.z)) // left
				tempX = -Time.fixedDeltaTime;
			

			if (dir2.z > 0 && Mathf.Abs(dir2.z) > Mathf.Abs(dir2.x)) // forward
				tempY = Time.fixedDeltaTime;
			if (dir2.z < 0 && Mathf.Abs(dir2.z) > Mathf.Abs(dir2.x)) // back
				tempY = -Time.fixedDeltaTime;

			if (dir.x > 0 && Mathf.Abs (dir.x) > Mathf.Abs (dir.z)) { // turn right
				var force = (turnForcePercent - Mathf.Abs (hMove.y)) * HelicopterModel.mass;
				HelicopterModel.AddRelativeTorque (0f, force, 0);
			}
			if (dir.x < 0 && Mathf.Abs (dir.x) > Mathf.Abs (dir.z)) { // turn left
				var force = -(turnForcePercent - Mathf.Abs(hMove.y)) * HelicopterModel.mass;
				HelicopterModel.AddRelativeTorque(0f, force, 0);
			}
		}
